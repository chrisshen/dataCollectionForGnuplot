...
vector<double>
STMAC_LineDataToVec(string line) {
...
main(){
	...
	***************************************/
	...
	/**************************************
		STMAC data
	***************************************/
	char dir_name[] = "/home/chrisomnet/usrtmac/sim_data_170228/EDCAd";
	...
	STMAC
	*/
	else if ( dir_name_string.find("STMAC") != string::npos){
		dataType = "STMAC";
	}
	else if ( dir_name_string.find("WPCF") != string::npos){
		dataType = "WPCF";
	}
	else if ( dir_name_string.find("LMA") != string::npos){
		dataType = "LMA";
	}
	else if ( dir_name_string.find("DMMAC") != string::npos){
		dataType = "DMMAC";
	}
	else if ( dir_name_string.find("dmmac_crossmap") != string::npos){
		dataType = "DMMAC_crossmap";
	}
	else if ( dir_name_string.find("EDCAn") != string::npos){
		dataType = "EDCAn";
	}
	else if ( dir_name_string.find("EDCAd") != string::npos){
		dataType = "EDCAd";
	}
	else {
		dataType = "error";
		throw runtime_error("L:636, Can't identify dataType!");
	}
	...
	/* STMAC */
	dataMapVecType dataAvgE2EDelay_STMAC;
	dataMapVecType dataMaxE2EDelay_STMAC;
	dataMapVecType dataPktRatio_STMAC;
	...
	/* STMAC */
	STMAC(entry, dp, dataPktRatio_STMAC, dataMaxE2EDelay_STMAC, dataAvgE2EDelay_STMAC);
	...
	if ( dataPktRatio_STMAC.size() != 0)
		meanPktRatio = computeAverage(dataPktRatio_STMAC);

	// dataMapType meanMaxE2EDelay;
	// if ( dataMaxE2EDelay_STMAC.size() != 0)
	// 	meanMaxE2EDelay = computeAverage(dataMaxE2EDelay_STMAC);

	dataMapType meanAvgE2EDelay;
	if ( dataAvgE2EDelay_STMAC.size() != 0)
		meanAvgE2EDelay = computeAverage(dataAvgE2EDelay_STMAC);
	...
	variMeanPktRatio = computeVariance(dataPktRatio_STMAC, meanPktRatio);

	// dataMapType variMeanMaxE2EDelay;
	// variMeanMaxE2EDelay = computeVariance(dataMaxE2EDelay_STMAC, meanMaxE2EDelay);

	dataMapType variMeanAvgE2EDelay;
	variMeanAvgE2EDelay = computeVariance(dataAvgE2EDelay_STMAC, meanAvgE2EDelay);
	...
	confiIntervalMeanPktRatio = computeConfiInterval(dataPktRatio_STMAC, variMeanPktRatio);

	// dataMapType confiIntervalMeanMaxE2EDelay;
	// confiIntervalMeanMaxE2EDelay = computeConfiInterval(dataMaxE2EDelay_STMAC, variMeanMaxE2EDelay);

	dataMapType confiIntervalMeanAvgE2EDelay;
	confiIntervalMeanAvgE2EDelay = computeConfiInterval(dataAvgE2EDelay_STMAC, variMeanAvgE2EDelay);
	...
	// get CDF
	dataMultiMapType cdfDataPktRatio_STMAC;
	cdfDataPktRatio_STMAC = getCDF(dataPktRatio_STMAC);

	// dataMultiMapType cdfDataMaxE2EDelay_STMAC;
	// cdfDataMaxE2EDelay_STMAC = getCDF(dataMaxE2EDelay_STMAC);

	dataMultiMapType cdfDataAvgE2EDelay_STMAC;
	cdfDataAvgE2EDelay_STMAC = getCDF(dataAvgE2EDelay_STMAC);
	...
	// chdir( "/home/chrisomnet/usr/STMAC_gnuplot" );
	// chdir( "/home/chris/usr/RTP/saint/sim/data/plot/plotData" );
	chdir( "/home/chrisomnet/usrtmac/sim_data_170228/gnuplot_20170524" );
	...
	******************/
	...
	saveDataToFile(fileName, cdfDataPktRatio_STMAC);
	...
	// saveDataToFile(fileName, cdfDataMaxE2EDelay_STMAC);
	...
	saveDataToFile(fileName, cdfDataAvgE2EDelay_STMAC);
...
STMAC(	struct dirent 	*entry, 
	...
	while( (entry = readdir(dp)) != NULL ){
		...
		if ( !S_ISDIR(statbuf.st_mode) ){
			...
			cout << "file: " << dirName << endl;
			// if ( dirName.find("TMAC") != std::string::npos ){
			// 	// cout << "TMAC============================================" << endl;
			// 	STMAC_collectData(entry, dirName, dataPktRatio_STMAC, dataAvgE2EDelay_STMAC);
			// }
			if ( dirName.find("WPCF") != std::string::npos ){
				// cout << "WPCF============================================" << endl;
				STMAC_collectData(entry, dirName, dataPktRatio_STMAC, dataAvgE2EDelay_STMAC);
			}
			else if ( dirName.find("LMA") != std::string::npos ){
				// cout << "LMA============================================" << endl;
				STMAC_collectData_forLMA(entry, dirName, dataPktRatio_STMAC, dataAvgE2EDelay_STMAC);
			}
			else if ( dirName.find("DMMAC") != std::string::npos ){

			}
			// else if ( dirName.find("STMAC-EDCA") != std::string::npos ){
			// 	// cout << "EDCA ======================================== " << endl;
			// 	STMAC_collectData_forEDCAn(entry, dirName, dataPktRatio_STMAC, dataAvgE2EDelay_STMAC);
			// }
			else if ( dirName.find("STMAC-EDCA-MPI") != std::string::npos ){
				// cout << "EDCA MPI ======================================== " << endl;
				STMAC_collectData_forEDCAd(entry, dirName, dataPktRatio_STMAC, dataAvgE2EDelay_STMAC);
			}
			else std::cout <<"Unable to open the directory! \n";
		}
	}
}

void
STMAC_collectData_forEDCAd(	struct dirent *entry,
							std::string dirName, 
							dataMapVecType& dataPktRatio_STMAC,
							dataMapVecType& dataAvgE2EDelay_STMAC){
	...
	if ( ( found = dirName.find("d") ) != std::string::npos ){
		...
		*/
		...
		std::ifstream testfile ( entry->d_name /*, std::ios::ate*/ );
		if (testfile.is_open()){
			vector<double> lineData;
			float lossRatio;
			float meanInterval;
			...
			while ( getline(testfile, line) ){	
				if ( line.find("Loss Ratio: ") != std::string::npos){
					cout << xAxis << ": " << "\t" << line << endl;
					string::size_type posStr;
					if ((posStr = line.find("Loss Ratio: ")) != std::string::npos){
						string s = line.substr(posStr+12, line.length()-1);
						// cout << s << endl;
						lossRatio = stof(s);
						cout << "lossRatio: " << lossRatio << endl;
						// lineData.push_back(d);
					}
				}
				else if ( line.find("Mean Interval: ") != std::string::npos ){
					cout << xAxis << ": " << "\t" << line << endl;
					string::size_type posStr;
					if ((posStr = line.find("Mean Interval: ")) != std::string::npos){
						string s = line.substr(posStr+15, line.length()-1);
						// cout << s << endl;
						meanInterval = stof(s);
						meanInterval += 0.1;
						cout << "meanInterval: " << meanInterval << endl;
						// lineData.push_back(d);
					}
				}
			}

			lineData.push_back(lossRatio);
			lineData.push_back(meanInterval);
			...
			***********************/
			...
			saveDelay( 0, lineData, dataPktRatio_STMAC, xAxis*0.1 );

			// random pkt max E2E delay
			// saveDelay( 1, lineData, dataMaxE2EDelay_STMAC, xAxis);

			// random pkt avg E2E delay
			saveDelay( 1, lineData, dataAvgE2EDelay_STMAC, xAxis*0.1);
...
void
STMAC_collectData_forEDCAn(	struct dirent *entry,
							std::string dirName, 
							dataMapVecType& dataPktRatio_STMAC,
							dataMapVecType& dataAvgE2EDelay_STMAC){
	...
	std::string::size_type found;
	float xAxis = 0.0;
	if ( ( found = dirName.find("n") ) != std::string::npos ){
		// get x axis
		int i = 0;
		int j = 2;
		// cout << dirName[found] << endl;
		...
		*/
		...
		if (testfile.is_open()){
			...
			***********************/
			// random pkt delivery ratio
			saveDelay( 0, lineData, dataPktRatio_STMAC, xAxis );

			// random pkt max E2E delay
			// saveDelay( 1, lineData, dataMaxE2EDelay_STMAC, xAxis);

			// random pkt avg E2E delay
			saveDelay( 1, lineData, dataAvgE2EDelay_STMAC, xAxis);
...
void
STMAC_collectData(	struct dirent *entry,
					std::string dirName, 
					dataMapVecType& dataPktRatio_STMAC,
					dataMapVecType& dataAvgE2EDelay_STMAC){
	...
	std::string::size_type found;
	float xAxis = 0.0;
	if ( ( found = dirName.find("den") ) != std::string::npos ){
		// get x axis
		int i = 0;
		int j = 4;
		// cout << dirName[found] << endl;
		...
		if (testfile.is_open()){
			...
			while ( getline(testfile, line) ){
				if ( line.find("loss ratio: ") != std::string::npos){
					cout << xAxis << ": " << "\t" << line << endl;
					string::size_type posStr;
					if ((posStr = line.find(",")) != std::string::npos){
						string s = line.substr(posStr+1, line.length()-1);
						// cout << s << endl;
						float d = stof(s);
						// cout << d << endl;
						lineData.push_back(d);
				...
				else if ( line.find("E2EDelay: ") != std::string::npos ){
					cout << xAxis << ": " << "\t" << line << endl;
					string::size_type posStr;
					if ((posStr = line.find(",")) != std::string::npos){
						string s = line.substr(posStr+1, line.length()-1);
				...
				else if ( line.find("End_to_End_Delay: ") != std::string::npos ){
					cout << xAxis << ": " << "\t" << line << endl;
					string::size_type posStr;
					if ((posStr = line.find(",")) != std::string::npos){
						string s = line.substr(posStr+1, line.length()-1);
			...
			***********************/
			...
			// random pkt avg E2E delay
			saveDelay( 2, lineData, dataAvgE2EDelay_STMAC, xAxis);
...
void
STMAC_collectData_forLMA(	struct dirent *entry,
							std::string dirName, 
							dataMapVecType& dataPktRatio_STMAC,
							dataMapVecType& dataAvgE2EDelay_STMAC){
					...
					*/
					...
					if (testfile.is_open()){
						...
						***********************/
						...
						// random pkt avg E2E delay
						saveDelay( 1, lineData, dataAvgE2EDelay_SCMAC, xAxis);
